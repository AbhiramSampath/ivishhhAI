...
from ai_models.ivish.memory_agent import get_context, update_context
...
_SUPPORTED_MODELS = ['gpt-4', 'llama3', 'phi3', 'mistral', 'sarvam', 'coqui', 'whisper']
_SAFE_MODE_PROMPT = "[SAFE MODE] Output must be respectful, safe, and appropriate"

class ChatRequest(BaseModel):
    """
    📌 Structured chat request with security validation
    """
...
class SecureChatEngine:
    ...
    - Sanitizes input
    - Applies safety and emotion detection
    - Generates AI response
    - Updates memory
    - Logs to blockchain
    - Integrates with downstream AI modules
    - Implements ZKP, rate limiting, and honeypot defenses
    """
    ...
    async def _build_secure_prompt(self, context: str, message: str, emotion: str, tone: str) -> str:
        """Safety-wrapped prompt construction with tone injection"""
        base_prompt = f"""
    ...
    async def _handle_unsafe_input(self, payload: ChatRequest, safety_result: dict):
    ...
    async def _handle_chat_failure(self, payload: ChatRequest, session_id: str, error: str):
...
async def chat_endpoint(request: Request, payload: ChatRequest):
    ...
    try:
        if not await verify_chat_access(payload.user_id, payload.zkp_proof):
        ...
        context = ""
        if MEMORY_ENABLED:
            context = await get_context(payload.user_id, privacy_filter=True)
